...
isbi_csv_file = "./data/labels_isbi_fake_with_multiple.csv"

isbi_image_path = "/mnt/lustre/liuxinglong/data/ISBI/isbi_preprocess/image"
prep_folder = "../prep_result_multiple"
...
def main():
    ...
    for idx, label_piece in label_csv.iterrows():
        ...
        token = imagename.index("_")
        token2 = imagename.rindex("_")
        ...
        if token == token2:
            ...
            # multiple data
            imagepid = imagename[:token]
            imagetime = imagename[token + 1:token2]
            imagetime_extend = imagename[token + 1:]
            extend_box_id = "{}_{}_bbox.npy".format(imagepid, imagetime_extend)
            target_processed_image_id = "{}_{}_clean.npy".format(imagepid, imagetime)
            label_id = "{}_{}".format(imagepid, imagetime_extend)
            file_id = "{}_{}.mhd".format(imagepid, imagetime)
            bbox_id = "{}_{}_pbb.npy".format(imagepid, imagetime_extend)
            lbb_id = "{}_{}_lbb.npy".format(imagepid, imagetime_extend)
